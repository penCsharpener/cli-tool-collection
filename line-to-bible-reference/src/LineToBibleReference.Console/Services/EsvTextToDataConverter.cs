...
public class EsvTextToDataConverter : ITextToDataConverter
{
    ...
    public EsvTextToDataConverter(IFileService fileService, AppSettings settings)
    {
    ...
    public async IAsyncEnumerable<BibleVerseModel> ConvertToBibleReferences()
    {
        ...
        await foreach (var line in _fileService.ReadByLineAsync(_settings.ConverterPathMapping["esv"]))
        {
